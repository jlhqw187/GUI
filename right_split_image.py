...
def mouse_callback(event, x, y, flags, param, ):
    if event == cv2.EVENT_LBUTTONDOWN:
        ...
        if(count):
            ...
            green_filter[:, :, 2] = 255  # 将绿色通道设置为255
            alpha = 0.5  # 透明度
...
parser.add_argument('-images', default=r'E:\gongzuo\jichangxiangmu\lushan_918\bg_add1025', help='指定图像文件夹的路径')
...
for i in range(1, 10):
    # full_folder_path = os.path.join(os.path.dirname(image_folder), f'{i}_nobg_full')
    # empty_folder_path = os.path.join(os.path.dirname(image_folder), f'{i}_nobg_empty')
    full_folder_path = image_folder + '_class' + f'/{i}_nobg_full'
    empty_folder_path = image_folder + '_class' + f'/{i}_nobg_empty'
...
# cv2.setWindowProperty(window_name, cv2.WND_PROP_nobg_fullSCREEN, cv2.WINDOW_nobg_fullSCREEN)
...
while True:
    ...
    image = cv2.imdecode(np.fromfile(image_files[current_image_index]),cv2.IMREAD_COLOR)
    cv2.setWindowTitle(window_name, image_name + f"                   {current_image_index+1}/{len(image_files)}")

    # if os.path.exists(image_folder + '_class/1_nobg_full/' + image_name + '_1.jpg') or os.path.exists(image_folder + '_class/1_nobg_empty/' + image_name + '_1.jpg'):
    if os.path.exists(image_folder + '_class/1_nobg_full/' + image_name + '.jpg') or os.path.exists(image_folder + '_class/1_nobg_empty/' + image_name + '.jpg'):
        existed = True
    else:
        existed = False
    ...
    if existed:
        for tmp in range(1, 10):
            # clicked[tmp] = os.path.exists(image_folder + f'_class/{tmp}_nobg_full/' + image_name + f'_{tmp}.jpg')
            clicked[tmp] = os.path.exists(image_folder + f'_class/{tmp}_nobg_full/' + image_name + f'.jpg')
            if clicked[tmp]:
    ...
    if key == ord(' '):
        ...
        if existed:
            for i in range(1, 10):
                if os.path.exists(image_folder + '_class' + f'/{i}_nobg_full/'+image_name+".jpg"):
                    os.remove(image_folder + '_class' + f'/{i}_nobg_full/'+image_name+".jpg")
                else:
                    os.remove(image_folder + '_class' + f'/{i}_nobg_empty/'+image_name+".jpg")
        for i in range(1, 10):
            ...
            if clicked[i]:
                # cv2.imwrite(image_folder + '_class' + f'/{i}_nobg_full/'+image_name+".jpg".encode("utf-8").decode(), clip)
                cv2.imencode(".jpg", clip)[1].tofile(image_folder + '_class' + f'/{i}_nobg_full/'+image_name+".jpg")
            else:
                # cv2.imwrite(image_folder + '_class' + f'/{i}_nobg_empty/'+image_name+".jpg".encode("utf-8").decode(), clip)
                cv2.imencode(".jpg", clip)[1].tofile(image_folder + '_class' + f'/{i}_nobg_empty/'+image_name+".jpg")
        # clicked = np.zeros((10), dtype=bool)
    elif key == ord('d'):
        # cv2.destroyWindow(window_name)
        current_image_index = (current_image_index + 1) % len(image_files)
